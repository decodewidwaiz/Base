...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        setLoading(true);
        const response = await fetch(`https://base-git-dev-waizalam90s-projects.vercel.app//product/product/${id}`);
        if (!response.ok) {
    ...
    const fetchRecommendations = async (category: string, currentId: string) => {
      try {
        const response = await fetch(`https://base-git-dev-waizalam90s-projects.vercel.app//product/products`);
        if (response.ok) {
          const data = await response.json();
          const filtered = data
